<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#tableTimbangan').DataTable({
            ...
            "footerCallback": function(row, data, start, end, display) {
                ...
                $(api.column(17).footer()).html(formatNumber(totalNetto));
            },
            "select": true,
            "processing": true,
            "serverSide": true,
            "retrieve": true,
            "ajax": {
                "url": "<?= base_url('weighbridge_priode_dsip/ajax_list') ?>",
                "type": "POST",
                "data": function(data) {
            ...
            buttons: [
                <?php if ($this->session->userdata('excel') == 1) { ?> {
                        ...
                        action: function previewData() {
                            ...
                            let start = $('#start').val();
                            let end = $('#end').val();
                            let url = "<?= base_url('report/phpExcel_wb_dsip_priode/') ?>" + start + "/" + end + "/" + no_ref;
                ...
                <?php if ($this->session->userdata('pdf') == 1) { ?> {
                        ...
                        action: function previewData() {
                            ...
                            let start = $('#start').val();
                            let end = $('#end').val();
                            let url = "<?= base_url('report/printByPriodeDsip/') ?>" + start + "/" + end + "/" + no_ref;
                            window.open(url, "_blank");
                        }
                    },
                <?php } ?>
    ...
    function listPO() {
        ...
        $.ajax({
            url: "<?= base_url('weighbridge_priode_dsip/get_po') ?>",
            method: "post",
            dataType: "json",
            data: {
                tgl_msk: $('#start').val(),
