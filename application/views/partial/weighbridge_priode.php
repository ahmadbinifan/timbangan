<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#tableTimbangan').DataTable({
            ...
            "footerCallback": function(row, data, start, end, display) {
                ...
                $(api.column(17).footer()).html(formatNumber(NettoSupp));
            },
            "select": true,
            "processing": true,
            "serverSide": true,
            "retrieve": true,
            "ajax": {
                "url": "<?= base_url('weighbridge_priode/ajax_list') ?>",
                "type": "POST",
                "data": function(data) {
            ...
            buttons: [
                <?php if ($this->session->userdata('excel') == 1) { ?> {
                        ...
                        action: function previewData() {
                            ...
                            let start = $('#start').val();
                            let end = $('#end').val();
                            let url = "<?= base_url('report/phpExcel_wb_priode/') ?>" + start + "/" + end + "/" + no_ref;
                ...
                <?php if ($this->session->userdata('pdf') == 1) { ?> {
                        ...
                        action: function previewData() {
                            ...
                            let start = $('#start').val();
                            let end = $('#end').val();
                            let url = "<?= base_url('report/printByPriode/') ?>" + start + "/" + end + "/" + no_ref;
                            window.open(url, "_blank");
                        }
                    },
                <?php } ?>
    ...
    function listPO() {
        ...
        $.ajax({
            url: "<?= base_url('weighbridge_priode/get_po') ?>",
            method: "post",
            dataType: "json",
            data: {
                tgl_msk: $('#start').val(),
